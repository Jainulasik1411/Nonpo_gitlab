...
public class Nonpo_Process_lineitemstable extends BaseClass {
	...
	private void LoginFunctionality() throws InterruptedException, AWTException, IOException {
		...
		for (int i = 0; i < 39; i++) {
		    thread(); // custom wait/sleep

		    // Step 1: Click the last (latest) "Show menu" button
		    List<WebElement> showMenus = driver.findElements(By.xpath("//*[text()='Show menu']"));
		    WebElement latestMenu = showMenus.get(showMenus.size() - 1);
		    jsclick(driver, latestMenu); // Using JS click to avoid issues

		    waitWithTimeout(driver, 20); // wait for dropdown to appear
		    Thread.sleep(150); // small pause

		    // Step 2: Click "Duplicate Row"
		    WebElement duplicate = driver.findElement(By.xpath("//*[text()='Duplicate Row']"));
		    jsclick(driver, duplicate); // JS click again

		    System.out.println("Duplicated row " + (i + 1));
		}
