...
public class Nonpo_Process_Functional extends BaseClass {
	...
	private void LoginFunctionality() throws InterruptedException, AWTException, IOException {
		...
		user.click();
		Thread.sleep(1000);
		user.sendKeys("fuction_npo@gmail.com");
		...
		try {
			pass.sendKeys("JS2!fjq#v~$RiHb1OcV~");
		...
		driver.findElement(By.xpath("//span[contains(text(), '100123') and contains(text(), 'Brillar Pte Ltd')]")).click();
		thread();
		...
		driver.findElement(By.xpath("//*[@aria-label='Search']")).sendKeys("MOB");
		
		threadhalf();

		driver.findElement(By.xpath("//*[text()='Mobile phone and Tablets']")).click();
		wait(2000);
		...
		buttonClick(qwe);
		wait(100);
		qwe.sendKeys("dceo_npo2@gmail.com");
		...
		try {
			qwer.sendKeys("hz~9N8$OxOUhUpRyrNkq");
		} catch (StaleElementReferenceException e) {
		}
	...
	//	thread();
		...
		buttonClick(b1);
		wait(100);
		b1.sendKeys("fuction_npo@gmail.com");
		...
		try {
			c1.sendKeys("JS2!fjq#v~$RiHb1OcV~");
		} catch (StaleElementReferenceException e) {
		}
